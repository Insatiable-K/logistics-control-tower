...
try:
    ...
    # -------------------------------------------------------------------------
    # Select cleaned columns FIRST, then rename — shipment_mapping also has raw
    # "container"/"sipl" columns, so renaming without selecting creates duplicate
    # column names and sm["container"] becomes a DataFrame (AttributeError on .str).
    sm = shipment_mapping[["container_id", "sipl_number", "po_number"]].rename(
