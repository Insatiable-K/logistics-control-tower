# -*- coding: utf-8 -*-
"""
build_dashboard_data.py — Invoice Compliance Engine (Redesigned)
...
OBJECTIVE:
  ...
  The system must represent CURRENT operational state, not historical lifecycle.

DESIGN DECISION:
  Invoice compliance is ONLY based on live operational shipments.
  Dependencies on historical data (Bookings, Open PO, Booking Events) are removed.

DATA SCOPE (5 sources only):
  Operational Data:
    - In-Transit List by SIPL
...
BUSINESS RULES:
  1. All shipments created from In-Transit + Inventory data only
  2. Container + SIPL are primary business keys; PO is supplemental
  3. Missing = no actual invoice AND no Pending placeholder
  4. Pending = bill_inv contains "PEND"
  5. Every unmatched invoice is audited and reported
  6. Bill Split compliance is scored PER-SIPL

CATEGORIES:
  Required (4): Ocean Freight (OF), Customs, Duty, Drayage
  Tracked separately: Accessorial charges (never gate status)
...
# Data sources for INVOICE COMPLIANCE (5 sources only, per redesign)
INVOICE_COMPLIANCE_SOURCES = [
    "in_transit",           # Operational: current container/SIPL state
    "inventory_intransit",  # Operational: SKU details within SIPL
    "bills",                # Financial: invoice registry
    "account_1275",         # GL: Capitalized Inventory Freight
    "account_1313",         # GL: Prepaid Container Freight
]

# Supporting data (kept for other dashboards: Booking, Container Movement)
OPERATIONAL_SOURCES = [
    "open_po",              # Booking dashboard
    "bookings",             # Booking dashboard
    "supplier_invoices",    # Shipment mapping (PO/SIPL/Container)
]
...
print("=" * 70)
print(f"\n  INVOICE COMPLIANCE: Using {len(INVOICE_COMPLIANCE_SOURCES)} data sources only")
print(f"    {', '.join(INVOICE_COMPLIANCE_SOURCES)}")

# =============================================================================
# LOAD + CLEAN ALL SOURCE FILES
# =============================================================================
print("\n--- LOADING AND CLEANING SOURCE FILES ---")
...
# --- Confirmed-complete lookup, needed BEFORE pending inference -----------
# BUG FIX: this must only ever be built from confirmed (non-pending) events.
# A prior version of this pivot didn't filter on is_pending, so any pending
# event for a (sipl, category) ALSO counted as "Complete" for that same
# pivot — which silently swallowed every pending signal before it could
# ever reach the Pending bucket. Confirmed via testing: pending_categories
# was always empty, not just thin on data.
confirmed_by_sipl = (
    gl_matched.dropna(subset=["sipl", "category"])
...
def _outstanding_categories(sipl_id):
    if sipl_id not in confirmed_by_sipl.index:
        return set(REQUIRED_CATEGORIES)
    row = confirmed_by_sipl.loc[sipl_id]
    return {cat for cat in REQUIRED_CATEGORIES if row[cat] == 0}

# --- Uncategorized pending activity -> category inference -----------------
# Per the documented Invoice Team workflow: Ocean Freight/Customs/Duty are
# normally filed together at customs clearance (Stage 2); Drayage is
# invoiced later, after delivery (Stage 4), so the Invoice Team enters a
# "Pending" placeholder for whichever category is still outstanding so the
# shipment can move through SPS. This means a pending bill's category can
# often be inferred WITHOUT guessing:
#
#   Rule A (vendor history): if the vendor's confirmed GL history is
#   unambiguously one category, use it.
#   Rule B (process of elimination): if a SIPL already has confirmed
#   invoices for all-but-one required category, any pending activity on
#   that SIPL can only be for the one category still outstanding — this is
#   exactly the Stage 3 Drayage scenario the business described, not a
#   statistical guess.
#
# If neither rule resolves it (e.g. 2+ categories still outstanding and no
# reliable vendor match), the pending bill is left unattributed rather than
# assigned arbitrarily — surfaced only as a generic "activity in progress"
# signal, never assumed to fix a specific category.
...
def infer_pending_category(row):
    vendor_cat = vendor_category_map.get(row["vendor"])
    outstanding = _outstanding_categories(row["sipl"]) if pd.notna(row["sipl"]) else set(REQUIRED_CATEGORIES)
    if vendor_cat is not None and vendor_cat in outstanding:
        return vendor_cat, "VENDOR_HISTORY"
    if len(outstanding) == 1:
        return next(iter(outstanding)), "PROCESS_OF_ELIMINATION"
    return None, None

inferred = pending_activity.apply(infer_pending_category, axis=1, result_type="expand")
pending_activity["inferred_category"], pending_activity["inference_method"] = inferred[0], inferred[1]

by_method = pending_activity["inference_method"].value_counts(dropna=False)
...
print(f"  Uncategorized pending activity : {len(pending_activity):,} total, "
      f"{inferred_count:,} category-inferred ({by_method.to_dict()}), "
...
# category — we don't guess when 2+ categories are still open.
...
pending_events["source_tier"] = "PENDING_" + pending_events["inference_method"]
...
# Reuse the confirmed-only pivot computed in STEP 4 (before pending
# inference needed it) — this is the bug-fixed version that correctly
# excludes pending rows.
complete_by_sipl = confirmed_by_sipl

# Pending coverage per (sipl, category), now correctly resolved via vendor
# history + process-of-elimination (STEP 4) rather than always empty.
