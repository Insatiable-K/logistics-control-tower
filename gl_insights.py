...
with col1:
    ...
    for cat in cat_1275.index:
        ...
        st.write(f"{cat or 'Uncategorized'}: ${amount:,.0f} ({pct:.0f}%)")

    fig = px.pie(
        values=cat_1275.values,
        names=cat_1275.index,
        title="Where GL 1275 money goes"
    )
...
with col2:
    ...
    fig = px.pie(
        values=cat_1313.values,
        names=cat_1313.index,
        title="Where GL 1313 money goes"
    )
    st.plotly_chart(fig, use_container_width=True)
...
with col1:
    ...
    fig = px.bar(
        x=vendor_1275.values,
        y=vendor_1275.index,
        orientation="h",
        title="Top vendors in GL 1275"
    )
...
with col2:
    ...
    fig = px.bar(
        x=vendor_1313.values,
        y=vendor_1313.index,
        orientation="h",
        title="Top vendors in GL 1313"
    )
    fig.update_layout(height=400)
    st.plotly_chart(fig, use_container_width=True)
...
if "balance" in gl.columns:
    ...
    with col1:
        ...
        if len(balance_1275) > 0:
            ...
            fig.update_layout(
                title="GL 1275 Asset Value Over Time",
                xaxis_title="Date",
                yaxis_title="$ Value",
                hovermode="x unified"
            )
    ...
    with col2:
        ...
        if len(balance_1313) > 0:
            ...
            fig.update_layout(
                title="GL 1313 Spending Over Time",
                xaxis_title="Date",
                yaxis_title="$ Value",
                hovermode="x unified"
            )
...
if "week" in gl.columns:
    ...
    with col1:
        fig = go.Figure()
        fig.add_trace(go.Scatter(
            x=postings_by_week.index.astype(str),
            y=postings_by_week.values,
            mode="lines+markers",
            name="Postings",
            line=dict(color="#9b59b6", width=2)
        ))
        fig.update_layout(
            title="How many freight costs recorded each week?",
            xaxis_title="Week",
            yaxis_title="Count",
            hovermode="x unified"
        )
    ...
    with col2:
        ...
        if len(amount_by_week_1275) > 0:
            fig.add_trace(go.Scatter(
                x=amount_by_week_1275.index.astype(str),
                y=amount_by_week_1275.values,
                mode="lines",
                name="GL 1275",
        ...
        if len(amount_by_week_1313) > 0:
            fig.add_trace(go.Scatter(
                x=amount_by_week_1313.index.astype(str),
                y=amount_by_week_1313.values,
                mode="lines",
                name="GL 1313",
                line=dict(color="#e74c3c", width=2)
            ))
        fig.update_layout(
            title="Total freight costs each week",
            xaxis_title="Week",
            yaxis_title="$ Amount",
            hovermode="x unified"
        )
...
st.markdown(
    ...
    🔗 **Next step**: This accounting data can be matched back to your actual bills and shipments
    to see what you're paying for and where costs can be optimized.
    """
)
