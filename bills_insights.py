...
Runs standalone (`streamlit run bills_insights.py`) or as a tab inside
...
def render_tab():
    """Render the full Bills Insights view. Callable standalone or as a tab."""
    ...
    try:
        ...
        total_containers = 0 if pd.isna(total_containers) else int(total_containers)
    except Exception:
        total_containers = 0
    ...
    with col1:
        st.metric("Total Billed", f"${total_billed:,.0f}", help="Total amount of all bills")
    with col2:
        st.metric("Average Bill", f"${avg_bill:,.0f}", help="Average cost per bill")
    with col3:
        st.metric("Still Owed", f"${still_owed:,.0f}", help="Amount still unpaid to vendors")
    with col4:
        st.metric("% Unpaid", f"{pct_owed:.0f}%", help="Percentage of total bills still unpaid")
    ...
    with col1:
        st.markdown("### 🟡 Waiting for Invoices")
        st.markdown(
            f"""
            **{pending} bills marked as "PENDING"** (${pending_amount:,.0f})
            ...
            Every pending bill is still tied to a known container — see the Container Insights tab
            to look up which container a specific pending bill belongs to.
            """
        )
        if pending > 0:
    ...
    with col2:
        st.markdown("### 🔴 Suspicious Invoice Numbers")
        st.markdown(
            f"""
    ...
    if st.button("📊 Download Cleaned Bills as CSV", key="bills_export_btn"):
