...
# -*- coding: utf-8 -*-
"""
FINAL Invoice Compliance ETL Pipeline
Correct logic: Use SIPL to fill blank containers before dropping
"""
...
print("=" * 80)
print("INVOICE COMPLIANCE ETL - FINAL (With SIPL Container Resolution)")
print(f"Run date: {datetime.today().date()}")
print("=" * 80)

# ============================================================================
# STEP 1: LOAD SHIPMENT DATA & BUILD SIPL->CONTAINER LOOKUP
# ============================================================================
print("\nSTEP 1: Build SIPL to Container lookup from shipment sources")
print("-" * 80)
...
# Build SIPL->Container lookup
sipl_to_container = {}

# From IN_TRANSIT
for idx, row in in_transit.iterrows():
    sipl = str(row['sipl']).strip()
    container = row['_container_clean']
    if pd.notna(container):
        if sipl not in sipl_to_container:
            sipl_to_container[sipl] = container

# From INVENTORY (secondary, if not already in lookup)
for idx, row in inventory.iterrows():
    sipl = str(row['sipl']).strip()
    container = row['_container_clean']
    if pd.notna(container) and sipl not in sipl_to_container:
        sipl_to_container[sipl] = container

print(f"  SIPL->Container mappings: {len(sipl_to_container)}")

# ============================================================================
# STEP 2: FILTER IN_TRANSIT TO INTERNATIONAL + TIMEFRAME
# ============================================================================
print("\nSTEP 2: Filter to international shipments within ETA window")
print("-" * 80)
...
print(f"  Distinct containers: {len(shipment_containers)}")
print(f"  Distinct SIPLs: {len(shipment_sipls)}")

# ============================================================================
# STEP 3: LOAD BILLS & RESOLVE BLANK CONTAINERS VIA SIPL
# ============================================================================
print("\nSTEP 3: Load Bills and resolve blank containers")
print("-" * 80)
...
print(f"  With container value: {bills['_container_clean'].notna().sum()}")
print(f"  Blank container: {bills['_container_clean'].isna().sum()}")

# For blank containers, try to resolve via SIPL
blank_mask = bills['_container_clean'].isna()
bills['_original_container_blank'] = False
bills['_filled_from_sipl'] = False
bills.loc[blank_mask, '_original_container_blank'] = True

# Attempt to fill blank containers using SIPL lookup
for idx in bills[blank_mask].index:
    sipl = str(bills.at[idx, 'sipl_inv']).strip() if pd.notna(bills.at[idx, 'sipl_inv']) else None

    if sipl and sipl in sipl_to_container:
        bills.at[idx, '_container_clean'] = sipl_to_container[sipl]
        bills.at[idx, '_filled_from_sipl'] = True

# Audit the fill operation
filled = bills['_filled_from_sipl'].sum()
still_blank = bills['_container_clean'].isna().sum()

print(f"\n  Blank containers resolved via SIPL: {filled}")
print(f"  Still blank after SIPL resolution: {still_blank}")

# Filter to rows with valid containers only (no blank)
bills_valid = bills[bills['_container_clean'].notna()].copy()

print(f"  Bills with valid containers: {len(bills_valid)}")

# ============================================================================
# STEP 4: MATCH BILLS TO SHIPMENT UNIVERSE
# ============================================================================
print("\nSTEP 4: Match bills to shipment universe")
print("-" * 80)
...
print(f"  Bills valid with containers: {len(bills_valid)}")
print(f"  Matched to shipments: {len(bills_matched)}")
print(f"  Match rate: {100*len(bills_matched)/len(bills_valid) if len(bills_valid) > 0 else 0:.1f}%")
...
# ============================================================================
# STEP 5: LOAD & CLASSIFY GL
# ============================================================================
print("\nSTEP 5: Load and classify GL accounts")
print("-" * 80)
...
# ============================================================================
# STEP 6: MATCH GL TO BILLS
# ============================================================================
print("\nSTEP 6: Match GL to bills via invoice number")
print("-" * 80)
...
print(f"  Matched to bills: {len(gl_matched)}")
print(f"  Match rate: {100*len(gl_matched)/len(gl_data):.1f}%")

if len(gl_matched) > 0:
    category_matched = gl_matched['_category'].value_counts()
    print(f"  Matched by category:")
    for cat in ['Ocean Freight', 'Customs', 'Duty', 'Drayage', 'Other']:
        count = category_matched.get(cat, 0)
        if count > 0:
            pct = 100 * count / len(gl_matched)
            print(f"    {cat}: {count} ({pct:.1f}%)")

# ============================================================================
# STEP 7: SCORE INVOICE COMPLIANCE PER SIPL
# ============================================================================
print("\nSTEP 7: Score invoice compliance per SIPL")
print("-" * 80)
...
Bills Processing:
  Total: {len(bills)}
  With container: {bills['_container_clean'].notna().sum()}
  Resolved via SIPL: {filled}
...
# Save results
invoice_compliance.to_csv('invoice_compliance_final.csv', index=False)
print(f"[OK] Saved to invoice_compliance_final.csv")
